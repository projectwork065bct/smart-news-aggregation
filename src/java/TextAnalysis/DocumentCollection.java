...
//        }
//        return score;
        int distinctWordsCount=overAll.getDistinctWordsCount();
        if(matrix.length<1){
            System.err.println("theres an error");
            return 0;
        }
       // System.out.println("seeddoc="+seedDoc+", "+distinctWordsCount+"candidate="+candidateDoc);
        for(int i=0;i<distinctWordsCount;i++){
            if(matrix[i][seedDoc]>0){
                score+=matrix[i][candidateDoc]*weight[i];
            }
        }
        return score;
    }
    ...
    public double getModifiedCosineSimilarity(int seedDoc,int candidateDoc){
        double score=0;
        /*
        ...
        score=(getModifiedDotProduct(seedDoc,candidateDoc)/
                (getModifiedModulus(seedDoc)*getModifiedModulus(candidateDoc)));
        return score;
    ...
    public int getModifiedDotProduct(int seedDoc, int candidateDoc){
        int score=0;
        int distinctWordsCount=overAll.getDistinctWordsCount();
        if(matrix.length<1){
            System.err.println("theres an error");
       ...
       // System.out.println("seeddoc="+seedDoc+", "+distinctWordsCount+"candidate="+candidateDoc);
        for(int i=0;i<distinctWordsCount;i++){
            if(true){
                score+=matrix[i][seedDoc]*matrix[i][candidateDoc]*weight[i];
            }
        }
        return score;
    }
    public double getModulus(int doc){
        ...
        for(int i=0;i<allDocuments.elementAt(doc).words.size();i++){
            w=w+(allDocuments.elementAt(doc).count.elementAt(i).count
    ...
    public double getModifiedModulus(int doc){
       ...
       // System.out.println("seeddoc="+seedDoc+", "+distinctWordsCount+"candidate="+candidateDoc);
        for(int i=0;i<distinctWordsCount;i++){
            if(true){
                score+=matrix[i][doc]*matrix[i][doc]*weight[i];
            }
        }
       // return score;
