...
public class QueriesForOutput {
    ...
    public String getRelatedArticles() {
        ...
        ResultSet rs = null;
        String sql = "SELECT all main_id, related_id, bm25_score, cosine_score, modified_cosine_score,idf_score, sub_category"
        ...
        try {
            while (rs.next()) {
                ...
                if (subCategoryTemp.compareTo(subCategory) != 0) {
                    subCategory = subCategoryTemp;
                    relatedArticles += "<br><br><tr><td colspan='3'><h1><center>SUB CATEGORY:</h1> <td><h1>" + subCategory + "</h1><td><td><td><td></tr>";
                    relatedArticles += "<tr><th>MAIN ARTICLE<th>RELATED ARTICLE<th>BM25 SCORE<th>COSINE SIMILARITY SCORE<th>MODIFIED COSINE SIMILARITY SCORE<th>IDF SCORE</tr>";
                }

                relatedArticles += "<tr><td>" + qfp.getTitleOfArticleId(rs.getInt("main_id")) + "<td>" + qfp.getTitleOfArticleId(rs.getInt("related_id")) + "<td>" + rs.getFloat("bm25_score") + "<td>" + rs.getFloat("cosine_score") + "<td>" + rs.getFloat("modified_cosine_score") + "<td>" + rs.getFloat("idf_score");
            }
        } catch (SQLException ex) {
            Logger.getLogger(QueriesForOutput.class.getName()).log(Level.SEVERE, null, ex);
        }
